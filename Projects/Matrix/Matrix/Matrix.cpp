...
Matrix & Matrix:: operator = (const Matrix & M) {
	if (this == &M)
		return *this; //защита от самокопирования	
	for (int i = 0; i < n; ++i)   //старый массив очищаю
		delete [] a[i];
	delete []a;
	n = M.n;                      //новая размерность
	m = M.m;
	a = new double * [n];
	for (int i = 0; i < n; ++i)   //новый массив данных
		a[i] = new double[m];	
	for(int i = 0; i < n; ++i)    //новые данные в массиве
		for(int j = 0; j < m; ++j)
			a[i][j] = M.a[i][j];
	return *this;
}
...
Matrix Matrix::operator + (const Matrix & M) const {
	...
	if (n == M.n && m == M.m) {
		...
		temp.exist = false;
	return temp;
	//если размеры не равны возвращает нулевую, если размеры равны то складывает в цикле
}
...
Matrix Matrix::operator - (const Matrix & M) const {
	...
	if (n == M.n && m == M.m) {
		...
		temp.exist = false;
	return temp;
	//если размеры не равны возвращает нулевую, если размеры равны то вычитает в цикле
}
...
double Matrix::Det() const {
	if ((n != m) || !exist) {
		cerr << "error" <<endl;
		return 0;
		//если не квадратная или нулевая
	}
